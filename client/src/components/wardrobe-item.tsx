...
import { Heart, HeartOff, Edit, Trash2, MoreHorizontal, Check, Clock, Calendar } from "lucide-react";
...
import { WearTrackingDialog } from "@/components/wear-tracking-dialog";
...
export default function WardrobeItem({ item, onEdit }: WardrobeItemProps) {
    ...
    const [showWearDialog, setShowWearDialog] = useState(false);
    const queryClient = useQueryClient();
    const updateItem = useUpdateWardrobeItem();
    const deleteItem = useDeleteWardrobeItem();

    // Log wear mutation with correct endpoint
    ...
    return (
        <>
                    ...
                    <div className="flex items-start justify-between gap-2">
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                                <DropdownMenuItem onClick={handleQuickWear} disabled={logWearMutation.isPending}>
                                    <Check className="h-4 w-4 mr-2" />
                                    {logWearMutation.isPending ? 'Logging...' : 'Worn Today'}
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => setShowWearDialog(true)}>
                                    <Calendar className="h-4 w-4 mr-2" />
                                    Log Wear with Details
                                </DropdownMenuItem>
            ...
            <WearTrackingDialog
                open={showWearDialog}
                onOpenChange={setShowWearDialog}
                item={{
                    id: item.id,
                    name: item.name,
