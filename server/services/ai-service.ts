...
function validateAndFilterWardrobeItems(items: any[]): any[] {
    const validItems = items.filter(item => {
        ...
        if (!hasRequiredFields) {
            ...
            }, 'Filtering out wardrobe item with missing required fields');
            return false;
        }
        ...
        try {
            ...
            if (isLastAttempt) {
                if (error instanceof Error) {
                    logger.error(error, `${operationName} failed after ${maxRetries} attempts`);
                } else {
                    logger.error({ errorMessage: String(error) }, `${operationName} failed after ${maxRetries} attempts`);
                }
            }
...
Format your response as JSON with an array of outfits, each containing:
{
  ...
  "confidenceScore": 87
        ...
        logger.debug({ content }, 'OpenAI response received');
        ...
        try {
            ...
            throw new AppError('Failed to parse outfit recommendations', 500);
        }
    } catch (error) {
        ...
        if (errorMessage.includes('network') || errorMessage.includes('timeout')) {
            throw new AppError('Unable to connect to AI service. Please check your internet connection and try again.', 503);
        }

        throw new AppError('Failed to generate outfit recommendations. Please try again later.', 500);
    }
}
...
export async function analyzeUserStyle({
    ...
    try {
        ...
        if (validItems.length === 0) {
            ...
            throw new AppError('Not enough wardrobe items to analyze style. Please add more items to your wardrobe.', 400);
        }
...
Format your response as JSON with:
{
  ...
  "recommendedItems": ["Item type 1", "Item type 2", ...]
        ...
        try {
            ...
            const errorMsg = parseError instanceof Error ? parseError.message : String(parseError);
            logger.error(`Error parsing OpenAI response: ${errorMsg}`);
            throw new AppError('Failed to parse style analysis', 500);
        }
    } catch (error) {
        if (error instanceof AppError) {
            throw error; // Re-throw API errors
        ...
        throw new AppError('Failed to analyze style. Please try again later.', 500);
    }
}
...
Format your response as JSON:
{
  ...
  "occasionReasoning": "Why this outfit works for the occasion"
        ...
        try {
            ...
            const errorMsg = parseError instanceof Error ? parseError.message : String(parseError);
            logger.error(`Error parsing OpenAI response: ${errorMsg}`);
            throw new AppError('Failed to parse occasion outfit recommendation', 500);
        }
    } catch (error) {
        if (error instanceof AppError) {
            throw error; // Re-throw API errors
        ...
        throw new AppError('Failed to generate occasion outfit. Please try again later.', 500);
    }
}
...
export async function createUserStyleProfile(wardrobeItems: any[]) {
    try {
        ...
        throw new AppError('Failed to create style profile. Please try again later.', 500);
    }
}
